...
parser.add_argument('--checkpoint_directory', type = str, default = 'checkpoints_cifar-10_SimCLR', help = 'The path to store the model checkpoints')
...
writer = SummaryWriter('logs_cifar10_SimCLR')
...
train_dataset = torchvision.datasets.CIFAR10(root = args.dataset_path, train = True, 
	transform = TwoCropImageTransform(), download = True)
...
def run_epoch(data_loader, model, optimizer, epoch_count = 0):
	...
	for batch_id, (imgs, labels) in enumerate(data_loader):
		...
		simclr_loss = criterion(embeddings)

		optimizer.zero_grad()
		simclr_loss.backward()
		optimizer.step()

		running_loss = running_loss + simclr_loss.item()

		# Adding the logs in Tensorboard
		writer.add_scalar('SimCLR Loss', simclr_loss.item(), iter_count)
